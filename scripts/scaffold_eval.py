...
def unit_rows() -> list[dict]:
    ...
    for unit_id, reuse_of, wrong_sku in WRONG_ITEM_CASES:
        ...
        rows.append({
            ...
            "ordered_sku": wrong_sku, "ordered_asin": wrong_product[2], "image_files": "1.jpg;2.jpg",
            "_stage_note": f"No new photos -- copy the 2 photos from fixtures/{reuse_of}/ into this folder "
                           f"(same {product[3]}, but this row claims it was ordered as {wrong_product[3]}).",
        })
    for unit_id in BAD_PHOTO_UNITS:
        rows.append({
            "unit_id": unit_id, "org_id": ORG_ID, "order_id": f"ORD-EVAL-{unit_id}",
            "ordered_sku": PRODUCTS[0][1], "ordered_asin": PRODUCTS[0][2], "image_files": "1.jpg;2.jpg",
            "_stage_note": "Take 2 deliberately bad photos of ANYTHING: blurry, very dark, or just "
...
def write_photo_plan(rows: list[dict]) -> None:
    lines = [
        "# Photo plan\n",
        f"Generated by `scripts/scaffold_eval.py`. {len(rows)} units total. For each row, take 2 photos "
        "(a full view and a closer look at any damage/parts/label) and save them as `1.jpg` and `2.jpg` "
